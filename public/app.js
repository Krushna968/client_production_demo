...
/* ==========================================================================
   100+ Tasks Flow Section - Scroll-Triggered Step Reveal
   ========================================================================== */
function initTaskFlowAnimation() {
  const timeline = document.querySelector('.task-flow-timeline');
  const steps = document.querySelectorAll('.task-flow-step');
  if (!timeline || !steps.length) return;

  function updateProgress() {
    const rect = timeline.getBoundingClientRect();
    const vh = window.innerHeight;
    // 0 = top of timeline just entered bottom of viewport, 1 = fully scrolled past
    let progress = (vh - rect.top) / (rect.height + vh * 0.5);
    progress = Math.min(Math.max(progress, 0), 1);

    const segment = 1 / steps.length;
    steps.forEach((step, i) => {
      const stepProgress = Math.min(Math.max((progress - i * segment) / segment, 0), 1);
      step.style.setProperty('--reveal', stepProgress);
      step.classList.toggle('is-visible', stepProgress > 0.05);
      const fill = step.querySelector('.task-flow-connector-fill');
      if (fill) fill.style.height = (stepProgress * 100) + '%';
    });
  }
...
// Opens the existing Quote modal, temporarily relabeling its header
// (used by entry points like "Download the Task List" that reuse the same form)
window.openQuoteModalAs = function(headingText) {
  const heading = document.querySelector('#quoteModal .modal-wizard-header h3');
  const originalHeading = heading ? heading.innerText : null;

  if (heading && headingText) {
    heading.innerText = headingText;
  }

  toggleModal('quoteModal');

  // Restore the original heading once the modal is closed
...
window.calculateSavings = function() {
  ...
  const rates = {
    va: { outsource: 7, inhouse: 40 },
    bookkeeper: { outsource: 12, inhouse: 48 },
    developer: { outsource: 30, inhouse: 65 },
    marketer: { outsource: 22, inhouse: 50 },
    pa: { outsource: 10, inhouse: 35 }
  };
...
window.selectTime = function(btn, time) {
  ...
  const safeName = escapeHTML(defaultName);
  const safeEmail = escapeHTML(defaultEmail);
  ...
  if (confirmContainer) {
    confirmContainer.innerHTML = `
      <div class="scheduler-confirm-container" style="animation: fadeIn 0.3s ease; display: flex; flex-direction: column; gap: 10px; margin-top: 15px; background: rgba(0, 168, 150, 0.03); padding: 15px; border-radius: var(--radius-sm); border: 1px solid var(--border-light); text-align: left;">
        ...
        <input type="text" id="schedulerName" placeholder="Your Name *" class="form-control" style="padding: 8px 12px; font-size: 0.85rem; background: white;" value="${safeName}" required>
        <input type="email" id="schedulerEmail" placeholder="Work Email *" class="form-control" style="padding: 8px 12px; font-size: 0.85rem; background: white;" value="${safeEmail}" required>
        <button class="scheduler-confirm-btn" style="margin-top: 5px; width: 100%;" onclick="confirmBooking()">Confirm Free Trial Appointment</button>
      </div>
    `;
  }
};
  ...
  if (schedulerBody) {
    schedulerBody.innerHTML = `
      <div style="grid-column: span 2; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 24px; text-align: center;">
        ...
        <p style="color: var(--text-secondary); max-width: 450px; margin-bottom: 24px;">Your Trial call is booked for <strong>${escapeHTML(selectedDate)}</strong> at <strong>${escapeHTML(selectedTimeVal)}</strong> (EST). A calendar invite and Google Meet link have been dispatched to your email address.</p>
